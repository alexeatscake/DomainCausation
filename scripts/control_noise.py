...
Same blob centres in every panel; only the Gaussian noise changes (0.5, 1.0,
2.0). Each panel shows the scatter and the boundary lines (ERM, Controlling,
Domain classifier, Domain adversarial) fit on that panel's own data, titled with
its noise. Axis limits are shared so the panels are directly comparable.
...
# Same centres in every panel; only the noise changes. Matches the confounding
# case: imbalanced counts so class is correlated with domain.
BLOBS = [
...
def main() -> None:
    ...
    fig.tight_layout()
    save_path = Path(__file__).parent.parent / "figures" / "control_noise.png"
